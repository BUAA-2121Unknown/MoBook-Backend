...
from project.dtos.requests.upload_attachment_dto import UploadContentAttachmentDto, DownloadContentAttachmentSuccessData
...
from shared.utils.file.file_handler import parse_filename, save_file, construct_file_response, load_file, \
    save_file_by_content, load_file_by_content
...
@api_view(['POST'])
@csrf_exempt
def upload_artifact_content_attachment(request):
    user = get_user_from_request(request)
    if user is None:
        return UnauthorizedResponse(UnauthorizedDto())
    ...
    params = parse_param(request)
    try:
        dto: UploadContentAttachmentDto = deserialize(params, UploadContentAttachmentDto)
    ...
    # get file
    old_path = artifact.get_path()
    name, ext = parse_filename(dto.filename)
    artifact.filename = name
    artifact.extension = ext
    new_path = artifact.get_path()

    try:
        save_file_by_content(old_path, new_path, dto.content)
    except FileException as e:
        return InternalServerErrorResponse(InternalServerErrorDto(data=e))
...
@api_view(['GET'])
@csrf_exempt
def download_artifact_content_attachment(request):
    user = get_user_from_request(request)
    if user is None:
        return UnauthorizedResponse(UnauthorizedDto())
    ...
    try:
        ...
        if filename is None or filepath is None:
            raise FileException("File properties missing")
        content = load_file_by_content(filepath)
    except FileException as e:
        return NotFoundResponse(NotFoundDto(data=e))

    return OkResponse(OkDto(data=DownloadContentAttachmentSuccessData(
            artifact,
            filename,
            content)))
